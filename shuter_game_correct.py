...
# переменная "игра закончилась": как только True, в основном цикле перестают работать спрайты
finish = False
...
while run:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_SPACE:
                    # сколько выстеров сделано и не происходит ли перезарядка
                    ...
                    if num_fire  >= 5 and rel_time == False: # если игрок сделал 5 выстрелов
                        last_time = timer() # когда это произошло
    ...
    if not finish:
        ...
        # если спрайт коснулся врага - уменьшить жизнь
        ...
        # разный цвет в зависимости от количества жизней
        if life == 3:
            life_color = (0, 150, 0)
        if life == 2:
